...
export DATASET_NAME="/data2404/zhangchi/dataset/all_mri0602.csv" # 这里是一个csv，中间有两列，分别是image路径和对应的text
...
accelerate launch --num_processes=6 --mixed_precision="fp16" train_text_to_image.py \
  ...
  --lr_warmup_steps=100 \
  --validation_prompts "MRI, with tumor, glioma" "MRI, with tumor, MET Wild" "MRI, notumor" "MRI, coronal" "MRI, sagittal, with tumor, MET"\  # 可以放一些测试的prompts, 修改train_text_to_image.py的第165行内容
  --validation_epochs=1 \
